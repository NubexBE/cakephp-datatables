...
class Datatable
{
	...
	protected function processActionLinkList(array $sourceLinks): array
	{
		...
		foreach ($sourceLinks as $link) {
			if (Configure::read('executeMultitenantDatatableActionsCheck', false)) {
				if (isset($link['allowedOnChildSystem'])) {
					$allowedOnChildSystem = $link['allowedOnChildSystem']; // use this option to overwrite the default checks and always enable/disable a link
				} else {
